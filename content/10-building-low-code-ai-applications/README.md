...
[![Construire des applications IA à faible code](@img/10-lesson-banner.webp)](https://youtu.be/1vzq3Nd8GBA?si=h6LHWJXdmqf6mhDg)
...
1. Accédez à l'écran d'accueil de [Power Apps](https://make.powerapps.com).
...
![Décrire l'application que vous souhaitez créer](@img/copilot-chat-prompt-powerapps.webp)
...
1. L'assistant IA Copilot suggérera une table Dataverse avec les champs nécessaires pour stocker les données que vous souhaitez suivre, ainsi que des données d'exemple. Vous pouvez ensuite personnaliser la table selon vos besoins en utilisant la fonctionnalité d'assistant IA Copilot à travers des étapes conversationnelles.
   ...
   ![Champs suggérés dans votre nouvelle table](@img/copilot-dataverse-table-powerapps.webp)
...
![Ajout d'un nouveau champ](@img/copilot-new-column.webp)
...
![Ajout d'un nouvel écran via une instruction de prompt](@img/copilot-new-screen.webp)
...
![Sélectionner une nouvelle table](@img/describe-new-table.webp)
...
![Décrire la table](@img/copilot-chat-prompt-dataverse.webp)
...
![Table Dataverse suggérée](@img/copilot-dataverse-table.webp)
...
1. Accédez à l'écran d'accueil de [Power Automate](https://make.powerautomate.com).
...
2. Utilisez la zone de texte sur l'écran d'accueil pour décrire le flux de travail que vous souhaitez créer. Par exemple, **_Traiter une facture lorsqu'elle arrive dans ma boîte de réception_**. Cliquez sur le bouton **Envoyer** pour envoyer l'invite à l'IA Copilot.
   ...
   ![Copilot power automate](@img/copilot-chat-prompt-powerautomate.webp)
...
8. Supprimez l'action **Condition** du flux, car vous ne l'utiliserez pas. Cela devrait ressembler à la capture d'écran suivante :
   ...
   ![power automate, supprimer des actions](@img/powerautomate-remove-actions.webp)
...
11. Sélectionnez la **Table** que vous avez créée précédemment. Par exemple, vous pouvez sélectionner la table **Informations sur les factures**. Choisissez le contenu dynamique de l'action précédente pour remplir les champs suivants :
    ...
    ![power automate ajouter une ligne](@img/powerautomate-add-row.webp)
...
![créer une invite](@img/create-prompt-gpt.webp)

Pour apprendre à utiliser ce modèle d'IA dans Power Automate, consultez le module [Ajouter de l'intelligence avec AI Builder et GPT](https://learn.microsoft.com/training/modules/ai-builder-text-generation/).
...
Rendez-vous à la leçon 11 où nous examinerons comment [intégrer l'IA générative avec l'appel de fonction](@lesson/11-integrating-with-function-calling) !
